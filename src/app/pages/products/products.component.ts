...
export class ProductsComponent implements OnInit {
  ...
  public getAllProducts() {
    this.appService.getProducts('all').subscribe((res: any) => {
  ...
  public getSpecificProducts() {
    this.appService.getSpecificProducts(this.categoryName).subscribe((res: any) => {
      this.products = res.data;
      //for show more product  
      for (var index = 0; index < 3; index++) {
        this.products = this.products.concat(this.products);
  ...
  public onChangeCategory(event) {
    if (event.target) {
      ...
      this.appService.getSpecificProducts(event.target.innerText.toLowerCase()).subscribe((res: any) => {
        this.products = res.data;
        //for show more product  
        for (var index = 0; index < 3; index++) {
          this.products = this.products.concat(this.products);
